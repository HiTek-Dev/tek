...
<task type="auto">
  ...
  <action>
Plan 02 (Wave 1) has already extended identity-manager.ts so that `loadUser(agentId)` and `updateIdentityFileSection(file, section, content, agentId)` accept an optional agentId parameter. This task wires those through the gateway layer.
...
1. In `packages/gateway/src/tools/memory.ts`:
   - Change `createMemoryWriteTool()` signature to accept `agentId?: string` parameter:
     ...
     ```
   - In the `identity` case of the switch, pass agentId to `updateIdentityFileSection(identityFile, identitySection, content, agentId)` so writes go to the agent-specific directory
   - In `createMemoryReadTool`, update the `loadUser()` call to pass agentId: change `content = loadUser()` to `content = loadUser(agentId)` so USER.md reads come from the agent-specific directory
...
2. In `packages/gateway/src/agent/tool-registry.ts`, find where `createMemoryWriteTool()` is called and pass the `agentId`:
   ...
   - Also ensure `createMemoryReadTool()` passes `opts.agentId` if not already doing so.
