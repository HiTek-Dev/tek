...
4. In `packages/db/src/memory/identity-manager.ts`:
   ...
   - Add `agentId?: string` parameter to `loadUser()`:
     - Change the signature to `loadUser(agentId?: string)`
     - Use `resolveIdentityFile(agentId, "USER.md")` for agent-scoped resolution instead of always reading the global USER.md
     - This enables plan 01's memory_read tool to load USER.md from the correct agent directory
   - Add `agentId?: string` parameter to `updateIdentityFileSection()`:
     - Change the signature to `updateIdentityFileSection(file: string, section: string, content: string, agentId?: string)`
     - When agentId is provided, use `resolveAgentDir(agentId)` to resolve the file path instead of `GLOBAL_MEMORY_DIR`
     - This enables plan 01's memory_write tool to write identity files to agent-specific directories
